...
# Custom Modules
from utils import train_model, simple_train_model, save_history, combined_stats, create_data_loaders, create_data_loaders_transform, train_model_Cosine_Annealing, create_test_loaders, extract_model_arch_from_path, plot_corr_matrix
...
def main():
    ...
    if is_train:
        ...
        # Print the length of val_loader to check if it's created correctly
        ...
        model = MODEL_ARCH[model_arch](name=f"_b{batch_size}_lr{lr_str[-1]}_e{num_epochs}_{optimizer}_MERGE_cosine", dtype=torch.float32).to(device)
