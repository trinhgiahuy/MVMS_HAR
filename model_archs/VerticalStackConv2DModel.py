...
# Define the advanced Conv2D architecture with residual connections and SE blocks
class VerticalStackConv2DModel(nn.Module):
    def __init__(self, name, num_classes=7, dtype=torch.float32):
        super(VerticalStackConv2DModel, self).__init__()

        self.name = 'VerticalStackConv2DModel' + name
        self.dtype = dtype
        ...
        # Define Residual Blocks with SE blocks
        self.block1 = ResidualBlock(64, 64, stride=2).to(dtype=self.dtype)  # Downsample slightly more aggressively
        ...
        self.dropout = nn.Dropout(0.5).to(dtype=self.dtype)  # Increased dropout for better regularization
